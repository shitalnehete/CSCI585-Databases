...
\
SELECT i.Instructor FROM InstructorInfo i WHERE i.Subject IN (SELECT subject FROM Subjects) GROUP BY i.Instructor HAVING COUNT(*) = (SELECT COUNT(*) FROM Subjects);}